...
2. `Stamp/README.md` — 스탬프 이미지, 네이버페이 포인트 상품권 이미지, book count 업데이트
...
### 2-4. 네이버페이 포인트 상품권 이미지 추가 규칙

스탬프 누적 횟수가 **4의 배수**에 도달할 때 네이버페이 포인트 상품권 이미지를 추가합니다.

- 이번 meetup에서 받은 스탬프 중 4의 배수에 해당하는 스탬프가 있으면 네이버페이 포인트 상품권 이미지 삽입
- 네이버페이 포인트 상품권 이미지 파일: `NPayPoint5000.png`

**네이버페이 포인트 상품권 이미지 삽입 위치:**
- 4의 배수가 되는 스탬프 **바로 뒤**에 삽입
- 스탬프 2개 중 첫 번째 스탬프가 4의 배수 도달 → `stamp, 네이버페이 포인트 상품권, stamp`
- 스탬프 2개 중 두 번째 스탬프가 4의 배수 도달 → `stamp, stamp, 네이버페이 포인트 상품권`
- book 20/20 완료 시 → `stamp, 네이버페이 포인트 상품권, stamp` (cycle 완료 + 새 cycle 시작)
...
해당 meetup의 참여자는 comment 순서 기준으로 테이블 상단에 위치해야 합니다.

- **jongfeel**: 항상 첫 번째 행
- **해당 meetup 참여자**: jongfeel 바로 다음부터 comment 순서대로 배치 (기존 참여자라도 이번 meetup에 참여했으면 상단으로 이동)
- **비참여자**: 참여자 행 아래에 기존 순서 유지
- **신규 참여자**: 해당 meetup comment 순서에 따라 참여자 그룹 내 삽입
